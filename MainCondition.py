...
def train(modelConfig: Dict):
    ...
    # model setup
    net_model = UNet(T=modelConfig["T"], num_labels=10, ch=modelConfig["ch"], ch_mult=modelConfig["ch_mult"],
        num_res_blocks=modelConfig["num_res_blocks"], dropout=modelConfig["dropout"]).to(device)
    if modelConfig["load_weight"] is not None:
        net_model.load_state_dict(torch.load(os.path.join(modelConfig["save_dir"] ,modelConfig["load_weight"]), map_location=device), strict=False)
...
def eval(modelConfig: Dict):
    ...
    with torch.no_grad():
        ...
        print("labels: ", labels)
        model = UNet(T=modelConfig["T"], num_labels=10, ch=modelConfig["ch"], ch_mult=modelConfig["ch_mult"],
        num_res_blocks=modelConfig["num_res_blocks"], dropout=modelConfig["dropout"]).to(device)
        ckpt = torch.load(os.path.join(modelConfig["save_dir"], modelConfig["load_weight"]), map_location=device)
...
def main(state, model_config = None):
    modelConfig = {
        "epoch": 70,
        "batch_size": 64,
        "T": 500,
        ...
        "load_weight": "ckpt_83_.pt",
        "sampled_dir": "./SampledImgs/",
        "sampledNoisyImgName": "noisy.png",
        "sampledImgName": "sampled_80.png",
        "nrow": 8
    }
